...
const News = ({ news }: NewsProps) => {
  return (
    <div className="w-full flex flex-col items-center pt-28 px-5 sm:px-10">
      <div>
        ...
        <div className="hidden lg:flex gap-10">
          <div className="w-[860px] h-[488px] bg-[#020202] rounded-2xl cursor-pointer hover:scale-105 transition-transform duration-300">
            <img 
              src={news[0].bg} 
              alt="Game Poster" 
              className="h-[352px] w-full object-cover object-top rounded-t-2xl" 
            />
            ...
            {news.slice(1).map((item, i) => (
              ...
              >
                <img 
                  src={item.bg} 
                  alt="Game Poster" 
                  className="h-[142px] w-full object-cover object-top rounded-t-2xl" 
                />
        ...
        <div className="flex flex-wrap lg:hidden gap-5 justify-center">
          {news.map((item, i) => (
            <div 
              key={i} 
              className="w-full sm:w-[300px] h-56 bg-[#020202] rounded-2xl cursor-pointer hover:scale-105 transition-transform duration-300"
            >
              <img 
                src={item.bg} 
                alt="Game Poster" 
                className="h-[142px] sm:h-[160px] w-full object-cover object-top rounded-t-2xl" 
              />
              <div className="px-3 py-2 flex flex-col gap-1">
                <div className="text-[13px]">{item.title}</div>
                <div className="flex justify-between text-[#797979] text-[12px] w-full">
                  <span>{item.game}</span>
                  <span>{item.date}</span>
                </div>
              </div>
